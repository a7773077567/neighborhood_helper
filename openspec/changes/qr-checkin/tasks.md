...
- [x] 3.3 新增取消簽到 Server Action（`uncheckIn`）：接收 registrationId + eventId，將 attended 改回 false、清除 attendedAt
...
- [x] 5.1 建立 QR Code 顯示元件（QrCodeDisplay），接收 qrToken 並渲染 SVG QR Code
- [x] 5.2 建立 QrCodeButton（Client Component），點擊開啟 Dialog 顯示 QR Code + Wake Lock 防螢幕熄滅
- [x] 5.3 我的活動頁面（`/my-events`）EventCard 整合 QR 按鈕，條件式卡片結構（有 qrToken 時 div + Arrow Link，無時整張 Link）

## 6. 簽到頁面

- [x] 6.1 建立 QR 掃描元件（QrScanner），封裝 html5-qrcode 的相機啟動/停止/掃描回呼
- [x] 6.2 建立簽到頁面（`/admin/events/[id]/checkin`），整合掃描元件 + 簽到 Action + toast 回饋
- [x] 6.3 簽到頁面顯示進度條（已簽到/已報名人數）+ 完整報名名單（手動簽到/取消簽到按鈕），掃描後即時更新
- [x] 6.4 手機版掃描器開關（預設關閉，點擊開啟相機）+ 相機權限拒絕的提示訊息

## 7. Admin 後台整合

- [x] 7.1 活動管理列表（`/admin/events`）為 PUBLISHED 活動新增「簽到」操作按鈕
