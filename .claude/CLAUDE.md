...
不是所有改動都需要走完整流程。色彩調整、間距修改、小 bug fix 等輕量改動：

```
直接修改 → commit → 如果涉及視覺變更，檢查設計稿是否需要同步
```

**判斷標準**：
- 影響範圍 ≤ 2-3 個檔案 → Tweak
- 不涉及新的資料結構或 API → Tweak
- 其他 → Feature change（完整流程）
...
**Learn+ 模式**：
```
AI 解釋此 task 會用到的技術概念（2-3 分鐘）
→ AI 寫出完整程式碼，用註解標出關鍵決策點
→ 你逐行讀過，針對不理解的地方提問
→ AI 回答後，你決定要不要自己重寫某些部分
```
...
**Learn+ 模式**（預設模式）：
1. AI 解釋此 task 用到的技術概念（2-3 分鐘）
2. AI 寫出完整程式碼，用註解標出關鍵決策點
3. 你逐行讀過，針對不理解的地方提問
4. AI 回答後，你決定要不要自己重寫某些部分
...
**模式選擇**：
- AI 在每個 task 開始前主動詢問要用哪個模式
- 預設建議 Learn+，除非使用者表示對該技術已熟悉
- 能獨立實作 70% 以上 → Review，否則 → Learn+
